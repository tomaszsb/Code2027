#!/usr/bin/env python3
"""
Send message to Claude - Email format (.txt)
Version: 2.0 (Phase 1 Stabilization - Oct 7, 2025)

Usage:
    echo "Your message" | python3 send_to_claude.py MSG_TYPE
    python3 send_to_claude.py MSG_TYPE < message.txt

Message content is read from stdin.
Creates email-style .txt file in ai-bridge/.server/gemini-outbox/
"""
...
def send_to_claude(msg_type: str, content: str) -> str:
    """
    Send a message to Claude in email-style .txt format.
    ...
    # Resolve path: ai-bridge/.server/gemini-outbox/
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        print("Usage: send_to_claude.py MSG_TYPE < /path/to/message.txt", file=sys.stderr)
