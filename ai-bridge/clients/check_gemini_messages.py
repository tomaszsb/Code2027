#!/usr/bin/env python3
"""
Reliable script for Claude to check for new messages from Gemini.
Reads directly from the gemini-outbox/.unread directory.
...
LAST_MCP_VERSION_FILE = SERVER_DIR / ".claude-last-mcp-version"
...
def check_mcp_sdk_version():
    ...
    if current_version != last_version:
        ...
        print("   Consider re-testing the native 'read_gemini_messages()' tool.")
        print("-" * 60)
...
def get_new_messages() -> List[Dict[str, Any]]:
    ...
    if not GEMINI_OUTBOX_UNREAD.exists():
        return []

    new_messages = []
    json_files = sorted(GEMINI_OUTBOX_UNREAD.glob("*.json"), key=lambda f: f.stat().st_mtime)
    ...
    for file in json_files:
        try:
            ...
            GEMINI_OUTBOX_READ.mkdir(parents=True, exist_ok=True)
            shutil.move(str(file), str(GEMINI_OUTBOX_READ / file.name))
