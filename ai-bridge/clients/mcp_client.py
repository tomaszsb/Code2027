...
# Debug Log
DEBUG_LOG_PATH = SERVER_DIR / "mcp_client_debug.log"
...
# Directories
INBOX_DIR = SERVER_DIR / "gemini-outbox"  # Gemini's outbox is Claude's inbox
OUTBOX_DIR = SERVER_DIR / "claude-outbox"  # Claude's outbox
...
# JSON Schema for message validation - supports both old and compact formats
MESSAGE_SCHEMA_OLD = {
    "type": "object",
    "properties": {
        ...
        "type": {"type": "string"},  # Permissive - allow any message type
        "payload": {
            "type": "object",
            "properties": {
                "content": {"type": "string"},
...
MESSAGE_SCHEMA_COMPACT = {
    "type": "object",
    "properties": {
        "id": {"type": "string"},
        "to": {"enum": ["gemini", "claude"]},
        "t": {"type": "string"},
        "c": {"type": "string"}
    },
    "required": ["id", "to", "t", "c"]
...
def normalize_message(message: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize compact format to old format for internal processing"""
    if "id" in message:  # Compact format
        return {
            "message_id": message["id"],
            "timestamp": "",  # Not in compact format, derive from ID if needed
            "sender": "claude" if message["id"].startswith("claude") else "gemini",
    ...
    return message  # Already in old format

def validate_message(message: Dict[str, Any]) -> bool:
    """Validate message against JSON schema - supports both old and compact formats"""
    # Try compact schema first (new format)
    try:
        jsonschema.validate(instance=message, schema=MESSAGE_SCHEMA_COMPACT)
        return True
    ...
    try:
        jsonschema.validate(instance=message, schema=MESSAGE_SCHEMA_OLD)
        return True
    except jsonschema.exceptions.ValidationError as e:
        log_debug(f"Validation error (both schemas failed): {e}")
        return False
...
def process_message(filepath: Path) -> None:
    ...
    try:
        ...
        if not validate_message(raw_message):
            ...
            send_error(
                raw_message.get('message_id', raw_message.get('id', 'unknown')),
                f"Message failed validation: {filepath.name}"
            )
            print(f"✗ Malformed message: {filepath.name}")
            return

        # Normalize to common format for processing
        ...
        # Phase 2: ACK messages removed - relying on directory states for delivery confirmation
