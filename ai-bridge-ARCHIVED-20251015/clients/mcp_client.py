...
def send_message(msg_id: str, to_agent: str, subject: str, content: str) -> None:
    ...
    message_content = create_message_email_format(msg_id, "claude", to_agent, subject, content)
...
def send_ack(original_message_id: str) -> None:
    ...
    msg_id = f"claude-ack-{timestamp}"
    send_message(msg_id, "gemini", "ACK", f"Acknowledged message {original_message_id}")


def send_error(original_message_id: str, error_content: str) -> None:
    """Send error message"""
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    msg_id = f"claude-error-{timestamp}"
    send_message(msg_id, "gemini", "ERROR", f"Error processing {original_message_id}: {error_content}")
